<?php
echo "<center><img src=\"https://i.imgur.com/FCnPU2K.png\"></center>";
if (isset($msg)) {
    if (stripos($msg, "/start") === 0) {
        $menu[] = array(
            array("text" => "Informazioni 💬",
                "callback_data" => "list"),
        );
        $menu[] = array(
            array("text" => "👥Gruppo",
                "url" => "http://t.me/Weared"),
        );      
        $menu[] = array(
            array("text" => "➕Aggiungimi ad un gruppo➕",
                "url" => "http://t.me/wearedservice_bot?startgroup=true"),
        );
        if (isset($cbdata)) {
            cb_reply($cbid, "Ok", false, $cbmid, 'Ciao '.$nome .'!'.PHP_EOL .'@WearedService_bot è il Bot più completo per aiutarti a gestire facilmente e in sicurezza i tuoi gruppi!'.PHP_EOL .' '.PHP_EOL .' 👉🏻 Aggiungimi in un supergruppo e impostami come Amministratore per farmi entrare subito in azione!'.PHP_EOL .' '.PHP_EOL .'❓ QUALI SONO I COMANDI? ❓'.PHP_EOL .'Premi /help per vedere tutti i comandi e il loro funzionamento! ', $menu);
        } else {
            sm($chatID, 'Ciao '.$nome .'!'.PHP_EOL .'@WearedService_bot è il Bot più completo per aiutarti a gestire facilmente e in sicurezza i tuoi gruppi!'.PHP_EOL .' '.PHP_EOL .' 👉🏻 Aggiungimi in un supergruppo e impostami come Amministratore per farmi entrare subito in azione!'.PHP_EOL .' '.PHP_EOL .'❓ QUALI SONO I COMANDI? ❓'.PHP_EOL .'Premi /help per vedere tutti i comandi e il loro funzionamento! ', $menu);
        }
    }
    if (isset($cbdata)&& $cbdata == "list") {
        ...
        cb_reply($cbid, "Ok", false, $cbmid,"Weared Service 🇮🇹 è stato sviluppato in PHP7, online dal 27 Marzo 2020 ed in continuo aggiornamento! \n " , $menu);
    }
}
if ($chatID < 0) {
